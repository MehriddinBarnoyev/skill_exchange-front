...
export default function UserProfile() {
  ...
  const handleAddSkill = async () => {
    ...
    try {
      ...
      const newAddedSkill = await addSkill(token, user.id, newSkill);
      setNewSkill({ name: "", description: "" });
  
      await fetchSkills(); // Yangi skill qo'shilgandan keyin listni qayta yuklash
    } catch (err) {
    ...
    <div className="container mx-auto p-4 bg-gray-50 min-h-screen">
      <Card className="mb-8 shadow-sm">
        <CardHeader className="bg-gray-100">
          <CardTitle className="text-2xl font-bold text-gray-800">Profile Overview</CardTitle>
        </CardHeader>
        <CardContent className="p-6">
          <div className="flex items-center space-x-4 mb-4">
            <Avatar className="w-24 h-24 border-2 border-gray-200">
              <AvatarImage src={user.profilePicture} alt={user.name} />
              <AvatarFallback className="bg-gray-200 text-gray-600">{user.name.charAt(0)}</AvatarFallback>
            </Avatar>
          ...
          {isOwnProfile &&
            (isEditing ? (
              <div className="space-x-2">
                <Button onClick={handleSave} className="bg-gray-700 hover:bg-gray-800">
                  Save
                </Button>
                <Button
                  variant="outline"
              ...
              <Button onClick={handleEdit} className="bg-gray-700 hover:bg-gray-800">
                Edit Profile
              </Button>
            ))}
        </CardContent>
      ...
      {isOwnProfile && (
        <Card className="mb-8 shadow-sm">
          <CardHeader className="bg-gray-100">
            <CardTitle className="text-2xl font-bold text-gray-800">My Skills</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            {!isShowingSkills ? (
              <Button onClick={fetchSkills} disabled={isSkillLoading} className="bg-gray-700 hover:bg-gray-800">
                {isSkillLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                Show My Skills
              </Button>
            ) : skills.length === 0 ? (
              <p className="text-gray-600">You haven't added any skills yet. Add your first skill to get started!</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                {skills.map((skill) => (
                  <Card key={skill.id} className="bg-white shadow-sm">
                    <CardHeader>
                      <CardTitle className="text-gray-700">{skill.name}</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p className="text-gray-600">{skill.description}</p>
                      <div className="mt-2 space-x-2">
                        <Button onClick={() => handleEditSkill(skill)} className="bg-gray-600 hover:bg-gray-700">
                          Edit
                        </Button>
                        <AlertDialog>
            ...
            {isShowingSkills && (
              <div className="mt-4">
                <Button onClick={() => setIsAddingSkill(true)} className="bg-gray-700 hover:bg-gray-800">
            ...
            {isAddingSkill && (
              <div className="space-y-2 mt-4">
                ...
                />
                <div className="space-x-2">
                  <Button onClick={handleAddSkill} disabled={isSkillLoading} className="bg-gray-700 hover:bg-gray-800">
                    {isSkillLoading ? (
      ...
      <AlertDialog open={!!editingSkill} onOpenChange={() => setEditingSkill(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleUpdateSkill} disabled={isSkillLoading}>
